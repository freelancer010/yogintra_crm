...
class Admin_roles extends CI_Controller
{
	...
	function index(){
		...
		$data['title'] = 'role_and_permissions';
	...
	public function module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(PANELURL.'admin_roles/module_add','refresh');
			}
			else{
				$data = array(
				...
				if($result){
					...
					redirect(PANELURL.'admin_roles/module');
				}
			}
		}
		else{
			$data['title'] = ('add_new_module');
	...
	public function module_edit($id=0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(PANELURL.'admin_roles/module_edit/'.$id,'refresh');
			}
			else{
				$data = array(
					'module_name' => $this->input->post('module_name'),
	...
	public function sub_module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(PANELURL.'admin_roles/sub_module_add/'.$parent,'refresh');
			}
			else{
				$data = array(
					'parent' => $parent,
				...
				if($result){
					...
					redirect(PANELURL.'admin_roles/sub_module/'.$parent);
				}
			}
		}
		else{
			...
			$this->load->view('includes/header');
			$this->load->view('admin_roles/sub_module_add', $data);
			$this->load->view('includes/footer');
		}
	}
	...
	public function sub_module_edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(PANELURL.'admin_roles/sub_module_edit/'.$id,'refresh');
			}
			else{
