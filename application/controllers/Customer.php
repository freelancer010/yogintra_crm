...
class Customer extends CI_Controller
{
	...
	public function savedata()
	{
		if ($this->input->post('name')) {
			...
			$data['is_customer'] = $this->input->post('is_customer') ? $this->input->post('is_customer') : '';
			$data['is_tellecalling'] = $this->input->post('is_tellecalling') ? $this->input->post('is_tellecalling') : '';
		...
		if($_SESSION['admin_role_id'] == 3){
			...
			$query = "SELECT `leads`.*,
					...
					ON `trainer`.`id` = `leads`.`trainer_id`
			 		...
			 		WHERE 1=1  AND `is_customer` = 1 
					AND `is_tellecalling` = 1 
					AND `status` = 1 {$where}
			...
			$query = "SELECT `leads`.*, `trainer`.`name` as `trainerName`, `trainer`.`created_date` as `trainer_created_date`, `trainer`.`number` as `trainer_number`, `trainer`.`salary` FROM `leads` LEFT JOIN `trainer` ON `trainer`.`id` = `leads`.`trainer_id`
			 WHERE 1=1  AND `is_customer` = 1 AND `is_tellecalling` = 1 AND `status` = 1 {$where}
			ORDER BY `created_date` DESC";
	...
	{
		$id = $_POST['id'];
		$resp = $this->db->where(['id' => $id])->update('leads', ['is_customer' => 0]);
		if ($resp) {
			$response = [
				'success' => 1,
	...
	public function changeStatusToTelecallingFromYogaCenter()
	{
		...
		$resp = $this->db->where(['id' => $lead_id])->update('leads', ['status' => 1]);
		if ($resp) 
		{
		    $this->db->where('id', $id);
		    $this->db->delete('yoga');
